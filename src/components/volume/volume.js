    ...
    useEffect(() => {
        const fetchVolumeData = async (data) => {
            try {
                ...
                const volumeData = await api.crud.update(request);
                ...
                if (volumeData.data.status === 200) {
                    ...
                    setIsLoading(false);
                    toastr.error('Error', volumeData.data.message);
                }
                if (volumeData.data.data.check_type === 'volume') {
                    navigate('/dashboard/volume');
                } else if (volumeData.data.data.check_type === 'distribution') {
